...
from utils.maths import Quaternion as Q
# is_pos_reward = True
...
class RacingEnv(DroneGymEnvsBase):
    def __init__(
        ...
        # self.file_path = 'datasets/spy_datasets/configs/racing/racing.scene_instance.json'
        ...
        self.orientations = th.as_tensor([
            [1.0, 0.0, 0.0, 0.0],    # 0度
            [0.7071, 0.0, 0.0, -0.7071], # 90度
            [0.0, 0.0, 0.0, -1.0],          # 180度
            [0.7071, 0.0, 0.0, 0.7071],  # 270度
        ], dtype=th.float32).to(self.device)
        self.yaw_target = th.as_tensor([
            [0],
            [-PI/2],
            [-PI+0.1],
            [PI/2],
        ...
        self.yaw = th.zeros((self.num_envs,), dtype=th.float32)
        self.theta = th.zeros((self.num_envs,), dtype=th.float32)
        ...
        # self.observation_space["noise_state"] = spaces.Box(
        #     low=-np.inf,
        #     high=np.inf,
        #     shape=(3 * (self._next_target_num - 1) + self.observation_space["state"].shape[0],),
        #     dtype=np.float32
        ...
        # self.observation_space["mask"] = spaces.Box(low=0, high=255, shape=(1,84,84,), dtype=np.uint8) 
        self.success_r = 5
    ...
    def _choose_target(self, indices=None):
        ...
        # for idx in indices:
        #     quat = self.orientation[idx]
        #     # 将四元数转换为欧拉角（偏航角yaw）
        #     r = R.from_quat([quat[0], quat[1], quat[2], quat[3]])  # 注意输入顺序为[x,y,z,w]
        #     yaw = r.as_euler('xyz', degrees=True)[0]
        #     print(f"Env {idx}: Target={self._next_target_i[idx]}, Yaw={yaw:.1f}°")
    ...
    #     reward = r_prog1 + r_crash  + r_pass + r_cmd + r_perc + r_anglev
    #     return reward 
    ...
    def get_reward(self) -> th.Tensor:
        ...
        # r_success = 10.0 * self.get_success()
        reward = r_prog1 + r_pass + r_perc + r_cmd + r_crash + r_anglev + r_col_avoid
        return reward 
...
class RacingEnv2(RacingEnv):
    ...
    def get_observation(
        ...
        theta_target = th.atan2(direction_vector[:, 1], direction_vector[:, 0])  # 弧度制
        theta_yaw = self.get_current_yaw()
        
        angle_diff = th.abs(theta_yaw - theta_target)
        self.angle_diff = th.min(angle_diff, 2 * PI - angle_diff)  # 确保范围在 [0, π]
        ...
        # noise_state = th.hstack([
        #     relative_pos_noisy/ self.max_sense_radius,
        #     self.orientation,
        #     self.velocity / 10,
        #     self.angular_velocity / 10,
        # ]).to(self.device)
