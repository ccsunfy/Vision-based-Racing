...
class Dynamics:
    ...
    def __init__(
        ...
        self._sensor_delay_steps = int(sensor_delay / ctrl_dt)
        ...
        # self._init_thrusts = th.ones((4, self.num), device=self.device) * self.hover_thrust_per_motor()
    ...
    def _init(self):
        ...
        self._acceleration = th.zeros((3, self.num), device=self.device)  # 线加速度
        ...
        self._pre_acc = [th.zeros((1, self.num), device=self.device)
            for _ in range(self._sensor_delay_steps)]
    def detach(self):
        ...
        self._accz = self._acceleration[2, :].clone().detach()  # 线加速度z分量
    ...
    def reset(
        ...
        if indices is None:
            ...
            self._ctrl_i = th.zeros((3, self.num), device=self.device)
            self._angular_acc = th.zeros((3, self.num), device=self.device)
            self._acceleration = th.zeros((3, self.num), device=self.device)
            self._accz = th.zeros((1, self.num), device=self.device)  # 线加速度z分量
            # self._pre_action = th.zeros((4, self.num), device=self.device)
            self._pre_action = [th.zeros(4, self.num) for _ in range(self._comm_delay_steps)]
            self._pre_acc = [th.zeros(1, self.num) for _ in range(self._sensor_delay_steps)]
            ...
            self._acceleration[:, indices] = th.zeros((3, len(indices)), device=self.device)
            self._accz[:, indices] = th.zeros((1, len(indices)), device=self.device)  # 线加速度z分量
            # self._pre_action[:, indices] = th.zeros((4, len(indices)), device=self.device)
            for i in range(self._comm_delay_steps):
                self._pre_action[i][:, indices] = self._pre_action[i][:, indices] * 0
            for i in range(self._sensor_delay_steps):
                self._pre_acc[i][:, indices] = self._pre_acc[i][:, indices] * 0
        self._pre_thr = self.thrusts.sum(dim=1)

        return self.state

    def step(self, action) -> Tuple[th.Tensor, th.Tensor]:
        # self._pre_action,action=action.T,self._pre_action.T
        ...
        # if self._sensor_delay_steps:
        #     self._pre_acc.append(self._acceleration.clone())
        #     self._acceleration = self._pre_acc[0].clone()
        #     self._pre_acc.pop(0)
        # else:
        #     self._acceleration = self._acceleration.clone()
        ...
        for _ in range(self._interval_steps):
            ...
            self._thrusts = self._thrusts / (1+0.049*velocity_body[2,:]) 
            # self._thrusts = self._thrusts * (1 - 0.02 * (th.arcsin((velocity_body[:2,:]).norm(dim=0, keepdim=True) / (velocity_body.norm(dim=0, keepdim=True) + 1e-6))).abs())  # 归一化 thrusts
            force_torque = self._B_allocation @ self._thrusts  # 计算力矩

            # compute linear acceleration and body torque
            # linear_drag = (
            #     self._drag_coeffs_2[0] * velocity_body[0, :].abs() * velocity_body[0, :].sign(),  # x
            #     self._drag_coeffs_2[0] * velocity_body[1, :].abs() * velocity_body[1, :].sign(),  # y
            #     self._drag_coeffs_2[0] * velocity_body[2, :].abs() * velocity_body[2, :].sign() * self.z_drag_coeffs  # z
            # )
            # linear_drag = th.stack(linear_drag, dim=0)
            # quadratic_drag = (
            #     self._drag_coeffs_2[1] * (velocity_body[0, :].pow(2)) * velocity_body[0, :].sign(),  # x
            #     self._drag_coeffs_2[1] * (velocity_body[1, :].pow(2)) * velocity_body[1, :].sign(),  # y
            #     self._drag_coeffs_2[1] * (velocity_body[2, :].pow(2) * velocity_body[2, :].sign() * self.z_drag_coeffs)  # z
            # )
            # quadratic_drag = th.stack(quadratic_drag, dim=0)
            
            linear_drag = th.tensor([[0.01,0.01,0.02]]).T * velocity_body
            ...
            drag = self._drag_coeffs * (self._orientation.inv_rotate(self._velocity - 0).pow(2)) + linear_drag
            acceleration =  self._orientation.rotate(z * force_torque[0] - drag) / self.m + g  
            # acceleration = self._orientation.rotate(z * force_torque[0] - drag) / self.m + g
            # acceleration = self._orientation.rotate(z * force_torque[0] - linear_drag - quadratic_drag) / self.m + g
            # acceleration = (z * force_torque[0] - quadratic_drag) / self.m + g
            # quadratic_drag = self._drag_coeffs * (self._orientation.inv_rotate(self._velocity - 0).pow(2))
            # acceleration = self._orientation.rotate(
            #     z * force_torque[0] - quadratic_drag) / self.m + g
            
            torque = force_torque[1:]

            # integrate the state
            self._position, self._orientation, self._velocity, self._angular_velocity, self._angular_acc, self._acceleration = (
            ...
            # self._accz = self._acceleration[2, :].clone().detach()  # 更新线加速度z分量
    ...
    def _get_thrust_from_cmd(self, command) -> th.Tensor:
        ...
        if self.action_type == ACTION_TYPE.THRUST:
            ...
            thrust_error = command[0] - (self._acceleration[2,:]+9.8) * self.m # z-acceleration error
            d_thr = (self.thrusts.sum(dim=1) - self._pre_thr) / self.ctrl_dt
            # body_torque_des = (
            ...
            # + (self._inertia @ self._angular_velocity)
            thrust_des =  \
                self._thrust_PID.p * thrust_error + command[0] +  self._thrust_PID.d * d_thr  # z-acceleration error
            thrusts_torque = th.cat([thrust_des.unsqueeze(0), body_torque_des])
    ...
    def load(self, path=""):
        ...
        self._thrust_PID = PID(p=th.tensor(data["THRUST_PID"]["p"]),
                               i=th.tensor(data["THRUST_PID"]["i"]),
                                d=th.tensor(data["THRUST_PID"]["d"]))
