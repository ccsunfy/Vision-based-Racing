...
class RacingEnv(DroneGymEnvsBase):
                            ...
                            [
                                {
                                    "class": "Uniform",
                                    "kwargs":
                                        {"position": {"mean": [2., 3., 1], "half": [0.3, 0.3, 0.3]},
    ...
    def get_reward(self) -> th.Tensor:
        ...
        r_vel = -lambda7 * ((self.velocity).norm(dim=1)-self.v_d).abs()
        ...
        # reward = r_prog1 + r_pass + r_perc + r_cmd + r_crash + r_col_avoid
        reward = r_prog1 + r_pass + r_perc + r_cmd + r_crash + r_height + r_col_avoid
        return reward 
    ...
    def get_observation(
        ...
        if not self.requires_grad:
            if self.visual:
                ...
                noisy_depth = 1.0 / th.as_tensor(depth_data).clamp(0.2, 10) + th.randn_like(th.as_tensor(depth_data)) * 0.05 
                # noisy_depth = th.as_tensor(depth_data)
                # noisy_depth = noisy_depth * (1 + th.rand_like(th.as_tensor(depth_data)) * 0.03)  # 添加噪声
                return TensorDict({
