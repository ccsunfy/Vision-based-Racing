...
class RacingEnv(DroneGymEnvsBase):
    ...
    def get_success(self) -> th.Tensor:
        ...
        return self._past_targets_num == len(self.targets) - 1
    ...
    #     # r_direction = lambda_dir * th.exp(-self.theta**4)
    ...
    #     r_col_avoid = -lambda7 * 1 / (self.collision_dis + 0.2) 
    ...
    #     reward = r_prog1 + r_pass + r_perc + r_cmd + r_crash + r_anglev
    #     return reward 
    ...
    #     r_pass = 5.0 * self.is_pass_next
    #     safe_height = 0.7  
    #     r_height = -lambda8 * (safe_height - self.position[:, 2]).clip(min=0)
    #     reward = r_prog1 + r_perc + r_cmd
    #     return reward 
    ...
    #     # r_pass = (1.0 -(self.position - self.targets[_next_target_i_clamp]).norm(dim=1))* self.is_pass_next
    #     r_pass = 5.0 * self.is_pass_next
    #     reward = r_prog1 + r_pass + r_cmd + r_crash
    #     return reward 
    ...
    #     # r_pass = (1.0 -(self.position - self.targets[_next_target_i_clamp]).norm(dim=1))* self.is_pass_next
    #     r_pass = 5.0 * self.is_pass_next
    #     reward = r_prog1 + r_perc + r_cmd + r_anglev
    #     return reward 
    ...
    #     # r_pass = (1.0 -(self.position - self.targets[_next_target_i_clamp]).norm(dim=1))* self.is_pass_next
    #     r_pass = 5.0 * self.is_pass_next
    #     reward = r_prog1 + r_pass + r_perc + r_crash + r_anglev
    #     return reward 
    
    # # reward_without_progress
    def get_reward(self) -> th.Tensor:
        lambda1 = 0.95
        lambda2 = 0.05
        lambda3 = 0.005
        ...
        # r_pass = (1.0 -(self.position - self.targets[_next_target_i_clamp]).norm(dim=1))* self.is_pass_next
        r_pass = 5.0 * self.is_pass_next
        reward = r_pass + r_perc + r_crash + r_anglev + r_cmd
        return reward 
...
class RacingEnv2(RacingEnv):
    ...
    def get_observation(
        ...
        if not self.requires_grad:
            ...
            #     noisy_depth = 2.0 / th.as_tensor(depth_data).clamp(0.2, 10) + th.randn_like(th.as_tensor(depth_data)) * 0.05
