...
class RealEnv:
    def __init__(self, log_prefix=""):
        ...
        # self.model = ort.InferenceSession("src/sim_to_real/waypoint_model_3_6_v0.onnx")
        # self.model = ort.InferenceSession("src/sim_to_real/waypoint_state_v1.onnx")
        self.model = ort.InferenceSession("src/sim_to_real/waypoint_state_327.onnx")
        # self.model_path = 'src/sim_to_real/waypoint_test_2'

        # self.model = ppo.load(self.model_path,device="cpu")
        # self.model.policy.eval()
        # end_model = time.time()
        # print(f"模型加载耗时: {end_model - start_model:.4f} 秒")
        x2 = th.randn((1, 16), dtype=th.float32)
    ...
    def odom_callback(self, data):
        ...
        self.orientation = th.tensor([orientation.w, orientation.x, orientation.y, orientation.z], dtype=th.float32).to(self.device)
        if (orientation.w<0): 
    ...
    #     # self._is_pass_next = (self.position - self.targets[_next_target_i_clamp].squeeze()).norm(dim=1) <= self.success_radius
    #     self._is_pass_next =  ((self.position - self.targets[_next_target_i_clamp].squeeze()) ** 2).sum() <= self.success_radius ** 2
    ...
    def get_success(self) -> bool:
        ...
        self._next_target_i = self._next_target_i + self._is_pass_next
        self._past_targets_num = self._past_targets_num + self._is_pass_next
        return bool((self._next_target_i == len(self.targets)-1).all().item())  # Convert to Python bool
    ...
    def process(self):
            ...
            state = th.hstack([
                ...
                self.angular_velocity.unsqueeze(0) / 2,
            ]).to(self.device)
            # end_state = time.time()
            ...
            # depth_image = depth_image.reshape(1,1,64,64)
            # depth_image = th.full((1, 1, 64, 64), 5)
            state = state.reshape(1,-1)
            # self.v_d = self.v_d.reshape(1,-1)
            # self._next_target_i = self._next_target_i.reshape(1,-1)
            # self.latent = self.latent.reshape(1,-1)

            # obs = {
            #     "depth": depth_image.float(),
            #     "state": state.float(), 
            #     "vd": self.v_d.float(),
            #     "index": self._next_target_i.float(),
            ...
            # action, h = self.model.predict(obs)
            actions = self.model.run(None, obs)
            onnx_action = actions[0]
            onnx_action = np.clip(onnx_action,-1,1)

            command = self.de_normalize(th.as_tensor(onnx_action, device=self.device))
