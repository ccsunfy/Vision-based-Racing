...
class bag_plot:
    def __init__(self):
        ...
        # self.bag_file = '/home/suncc/suncc/My-research/Vision-based-Racing/pid_test_all_y2.bag'
        # self.bag_file = '/home/suncc/suncc/My-research/Vision-based-Racing/pid_test_all_z2.bag'
    ...
    # def _interpolate_data(self, orig_time, orig_data, dim):
    #     interp_data = np.zeros((len(self.base_time), dim))
    #     for i in range(dim):
    #         interp_func = interpolate.interp1d(
    #             orig_time, orig_data[:,i],
    #             kind='linear',
    #             fill_value=(orig_data[0,i], orig_data[-1,i]),
    #             bounds_error=False
    #         )
    #         interp_data[:,i] = interp_func(self.base_time)
    #     return interp_data
    ...
    def align_data(self):
        ...
        #     self.imu_time
        # ])
        # self.base_time = np.linspace(
        #     start=np.max([np.min(self.actions_time), 
        #                 np.min(self.action_drone_time),
        #                 np.min(self.state_bf_time),
        #                 np.min(self.imu_time)]),
        #     stop=np.min([np.max(self.actions_time),
        #                 np.max(self.action_drone_time),
        #                 np.max(self.state_bf_time),
        #                 np.max(self.imu_time)]),
        #     num=int((np.max(all_timestamps)-np.min(all_timestamps))*200) # 100Hz采样率
        # )
        # self.start_time = self.base_time[0]
        # self.end_time = self.base_time[-1]
    ...
    def plot(self):
        ...
        figx, axsx = plot.subplots(figsize=(35, 10), nrows=1, ncols=1)
        # axsx.plot(self.ori_action_time, self.ori_action[:,1], label="cmd_real", color=colors[0])
        # axs.plot(self.sim_t, self.action_all.numpy()[:,1], label="cmd_sim", color=colors[3])
        axsx.plot(self.imu_time, self.imu_angle_v[:,0], linestyle='--',linewidth=2.0, label="anglex_real", color=colors[2])
        axsx.plot(self.sim_t, self.anglevel.numpy()[:,0], linewidth=2.0,label="anglex_sim",color=colors[1])
        axsx.grid(True, 
         color='gray', 
         linestyle='--', 
         linewidth=0.5, 
         alpha=0.7)  # alpha控制透明度)
        # figx.savefig("ctbr_x.png", dpi=1600)
        
        axsx.set_title(f"anglevelx")
        axsx.legend()
        ...
        # axs.plot(self.sim_t, self.action_all.numpy()[:,1], label="cmd_sim", color=colors[3])
        axsy.plot(self.imu_time, self.imu_angle_v[:,1],linestyle='--',linewidth=2.0, label="angley_real", color=colors[2])
        axsy.plot(self.sim_t, self.anglevel.numpy()[:,1], linewidth=2.0,label="angley_sim",color=colors[1])
        # figy.savefig("ctbr_y.png", dpi=1600)
        
        axsy.set_title(f"anglevely")
        axsy.legend()
        ...
        # axs.plot(self.sim_t, self.action_all.numpy()[:,1], label="cmd_sim", color=colors[3])
        axsz.plot(self.imu_time, self.imu_angle_v[:,2],linestyle='--',linewidth=2.0, label="anglez_real", color=colors[2])
        axsz.plot(self.sim_t, self.anglevel.numpy()[:,2],linewidth=2.0, label="anglez_sim",color=colors[1])
        # figz.savefig("ctbr_z.png", dpi=1600)
        
        axsz.set_title(f"anglevelz")
        axsz.legend()
