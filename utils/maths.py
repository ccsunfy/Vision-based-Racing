...
# def get_circle_target():
#     target = th.ones((training_params["num_env"], 1)) @ th.tensor([[9, 0., 1]])
#     return target
...
def pixel_to_body(pixel_coords, depth, camera_intrinsics, camera_extrinsics):
    ...
    coords = [body_coords[2], body_coords[0], -body_coords[1]]
    return coords
...
def world_to_camera(world_points, rvec, tvec):
    rotation = R.from_quat(rvec)
    r = rotation.as_matrix()
    camera_points = (r @ world_points.T).T + tvec
