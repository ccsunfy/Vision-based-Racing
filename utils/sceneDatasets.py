...
eye_pos_near = mn.Vector3(0.1, 0.5, 1)
...
class SceneManager(ABC):
    def __init__(
        ...
        if self.render_settings is not None:
            ...
            self.render_settings["axes"] = self.render_settings.get("axes", True)
    ...
    def set_pose(self, position, rotation):
        ...
        for scene_id in range(self.num_scene):
            for agent_id in range(self.num_agent_per_scene):
                ...
                self.trajectory[scene_id][agent_id].append(
    ...
    def _load_sensor(self) -> List[habitat_sim.sensor.SensorSpec]:
        ...
        for i, sensor_cfg in enumerate(self.sensor_settings):
            ...
            if self.random_settings is not None:
                if sensor_spec.uuid in self.random_settings.keys():
                    sensor_spec.noise_model = self.random_settings[sensor_spec.uuid].get("model")
                    sensor_spec.noise_model_kwargs = self.random_settings[sensor_spec.uuid].get("kwargs", {})
            sensor_cfgs_list.append(sensor_spec)
...
def debug():
    from utils.randomization import UniformRandomizer
    ...
    # a.reset()
    # pos, _ = a.get_pose()
    AverageRandomizers = UniformRandomizer(
    ...
    a.load_scenes()
    reset_pos, reset_ori, _, _ = AverageRandomizers.generate(num=num_agent_per_scene * num_scene)
    ...
    for i in pbar:
        ...
        for id, is_collisped in enumerate(is_collispeds):
            if is_collisped:
                ...
                reset_pos, reset_ori, _, _ = AverageRandomizers.generate(num=1)
